...
class MessageMongoRepositorySpec extends UnitSpec
  ...
  "repository" should {
    ...
    "save multiple notification for a client" in {
      ...
      val found = await(repository.collection.find(selector(clientId1)).cursor[ClientNotification]().collect[List](Int.MaxValue, Cursor.FailOnError[List[ClientNotification]]()))
      found.size shouldBe 2
      found should contain(client1Notification1)
      found should contain(client1Notification2)
    }
